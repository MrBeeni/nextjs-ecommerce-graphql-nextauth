...
interface detailsPropes {
  params: any;
}
const Details: FC<detailsPropes> = ({ params }) => {
  const id = params.id;
  ...
  useEffect(() => {
    if (products) {
      ...
      let review = Math.round(product?.rating?.rate);
      setDetails(product);

      // Create an array of SVG elements for star icons
      const stars: any = [];
      for (let i = 1; i <= 5; i++) {
        if (i <= review) {
          stars.push(
